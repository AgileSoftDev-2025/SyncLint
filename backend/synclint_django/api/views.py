...
# -------------------------------------------------------
# FUNGSI BARU MEMILIH ARTEFAK UNTUK DIBANDINGKAN
# -------------------------------------------------------
@login_required
@require_http_methods(['POST'])
def compare_artifacts_view(request):
    """
    Endpoint API untuk menerima dua ID artefak, mengambil 
    data filejson yang sudah diparsing, dan memanggil 
    engine perbandingan.
    """
    try:
        data = json.loads(request.body)
        artifact_id_1 = data.get('artifact_id_1')
        artifact_id_2 = data.get('artifact_id_2')

        # Validasi input
        if not artifact_id_1 or not artifact_id_2:
            return JsonResponse(
                {"status": "error", "errors": "Dibutuhkan dua ID artefak untuk perbandingan."},
                status=400
            )

        # 1. Ambil artefak dari database
        # Pastikan artefak ada dan dimiliki oleh user yang sedang login
        artefak1 = get_object_or_404(Artefak, id=artifact_id_1, workspace__user=request.user)
        artefak2 = get_object_or_404(Artefak, id=artifact_id_2, workspace__user=request.user)

        # 2. Ambil data 'filejson' yang sudah diparsing
        data1 = artefak1.filejson
        data2 = artefak2.filejson

        # Cek jika salah satu artefak belum diparsing
        if not data1 or not data2:
            return JsonResponse(
                {"status": "error", "errors": "Satu atau lebih artefak belum diparsing (filejson masih null)."},
                status=400
            )

        # 3. PANGGIL ENGINE PERBANDINGAN ANDA DI SINI
        # (Ini adalah placeholder, ganti dengan logika engine Anda)
        #
        # from engine.comparator import run_comparison 
        # result_engine = run_comparison(data1, data2)
        #
        # Untuk sekarang, kita buat hasil palsu (placeholder):
        placeholder_result = {
            "comparison_summary": f"Perbandingan sukses antara {artefak1.name} (tipe: {artefak1.type}) dan {artefak2.name} (tipe: {artefak2.type})",
            "inconsistencies": [
                {"type": "Major", "message": "Aktor 'Customer' di Use Case tidak ditemukan di BPMN Lane."},
                {"type": "Minor", "message": "Nama proses 'Login' di BPMN berbeda dengan 'User Login' di Use Case Spec."}
            ],
            "suggestions": ["Tambahkan Lane 'Customer' di BPMN.", "Samakan terminologi nama proses."]
        }
        # ===============================================

        # 4. (Opsional) Simpan hasil ini ke model 'Report'
        # report = Report.objects.create(result=json.dumps(placeholder_result))
        # report.artefaks.add(artefak1, artefak2)
    ...
    except Artefak.DoesNotExist:
        return JsonResponse({"status": "error", "errors": "Satu atau lebih artefak tidak ditemukan."}, status=404)
    except Exception as e:
        return JsonResponse({"status": "error", "errors": str(e)}, status=500)    